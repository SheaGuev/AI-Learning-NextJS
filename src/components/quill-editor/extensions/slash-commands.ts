...
export default class SlashCommands {
    ...
    try {
      ...
      this.quill.keyboard.addBinding({
        ...
        handler: (range: any, context: any) => {
          ...
          // Otherwise let default arrow behavior occur
          return true;
        }
      });
      ...
      // Make the Enter binding even higher priority and ensure it intercepts even in composition mode
      this.quill.keyboard.addBinding({
        key: 13, // Enter
        shiftKey: null, // Handle both with and without Shift
        priority: 9000, // Extremely high priority to override everything else
        handler: (range: any, context: any) => {
          // Only handle this binding if menu is open
          if (this.isOpen) {
            console.log('Enter key pressed with menu open - attempting to select item with highest priority');
            // Prevent the event from propagating to prevent insertions
            const result = this.handleEnter();
            return result;
          }
    ...
    if (slashIndex !== -1) {
      ...
      try {
        ...
        if (command.handler) {
          ...
          console.error('Command has no handler:', command.label);
        }
        ...
        } else if (command.label.toLowerCase().includes('checkbox')) {
          // Specifically handle checkbox format
          console.log('Inserting checkbox at index:', newIndex);
          this.quill.insertEmbed(newIndex, 'checkbox', false, 'user');
          this.quill.setSelection(newIndex + 1, 0, 'silent');
        }

        // Ensure the editor maintains focus
        setTimeout(() => {
          this.quill.focus();
          this.quill.setSelection(this.quill.getSelection()?.index || newIndex, 0, 'user');
        }, 0);
  ...
  handleArrowUp() {
    ...
    const selectedItem = this.container.querySelector('.ql-slash-command-item.selected');
    ...
    if (selectedItem) {
      ...
      (selectedItem as HTMLElement).style.outline = '';
    ...
    console.log('Arrow up: Selected item:', item.textContent);
  ...
  handleArrowDown() {
    ...
    console.log('Arrow down: Selected item:', item.textContent);
  ...
  handleEnter() {
    if (!this.isOpen || !this.container) {
      console.log('Enter key pressed but menu is not open, allowing default behavior');
      return true; // Allow default behavior (new line)
    }
    
    console.log('ENTER KEY PRESSED WITH MENU OPEN! Attempting to select item...');
    
    try {
      // Check if container is actually visible in the DOM
      if (this.container.classList.contains('hidden')) {
        console.log('Container has hidden class despite isOpen=true, fixing inconsistency');
        this.isOpen = false;
        return true; // Allow default if the UI shows it's closed
      }
      
      // First check for a selected item with explicit selected class
      const selectedItem = this.container.querySelector('.ql-slash-command-item.selected') as HTMLElement;
      
      if (selectedItem) {
        console.log('Found selected item with text content:', selectedItem.textContent);
        
        // Get the command label directly from the item
        const commandLabel = selectedItem.querySelector('.ql-slash-command-label')?.textContent;
        
        if (commandLabel && this.options.commands) {
          // Find the matching command
          const matchingCommand = this.options.commands.find(
            (cmd: CommandOption) => cmd.label === commandLabel
          );
          
          if (matchingCommand) {
            console.log('Found matching command:', matchingCommand.label);
            
            // Force Quill to focus 
            this.quill.focus();
            
            // Execute the command directly
            this.executeCommand(matchingCommand);
            
            // Close menu and prevent default behavior
            this.closeMenu();
        ...
        // If we couldn't find the command, try simulating a click
        console.log('Dispatching click event on selected item');
        selectedItem.click();
        
        this.closeMenu();
        return false;
      } else {
        console.log('No explicitly selected item found, looking for first visible item');
      }
      
      // If no explicitly selected item, fall back to the first visible item
      const firstVisibleItem = this.container.querySelector('.ql-slash-command-item:not([style*="display: none"])') as HTMLElement;
      
      if (firstVisibleItem) {
        console.log('Using first visible item:', firstVisibleItem.textContent);
        firstVisibleItem.click();
        this.closeMenu();
        return false;
      }
      
      // If all else fails, close the menu
      console.log('No items found to select, closing menu');
      this.closeMenu();
    } catch (error) {
