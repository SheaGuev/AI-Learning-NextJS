// Flashcard implementation for Quill
import { withDisabledMarkdown } from '../utils/markdown-helpers';
...
export const FlashcardBlot: FlashcardBlotStatic = {
      ...
      frontContent.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'Delete' || e.key === 'Backspace') {
          ...
          if (frontContent.innerText.length === 0 || 
            ...
            e.preventDefault(); // Prevent deletion if content is empty
          }
  ...
  value(node: HTMLElement) {
    ...
    // Find all pairs of front/back elements
    const fronts = node.querySelectorAll('.ql-flashcard-front');
    const backs = node.querySelectorAll('.ql-flashcard-back');
    
    fronts.forEach((front, i) => {
      const frontContent = front.querySelector('.ql-flashcard-content')?.innerHTML || '';
      const backContent = backs[i]?.querySelector('.ql-flashcard-content')?.innerHTML || '';
...
// Register the flashcard blot with Quill
